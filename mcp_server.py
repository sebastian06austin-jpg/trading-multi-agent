...
def update_portfolio(action: str, ticker: str, qty: float, price: float, reason: str) -> str:
    ...
    port["trade_log"].append({"time": datetime.now(pytz.timezone("Asia/Kolkata")).isoformat(), "action": action, "ticker": ticker, "qty": qty, "price": price, "reason": reason})
    with open(path, "w") as f:
...
async def health_check(request):
    return JSONResponse({"status": "mcp_healthy", "time": datetime.now(pytz.timezone("Asia/Kolkata")).isoformat()})
